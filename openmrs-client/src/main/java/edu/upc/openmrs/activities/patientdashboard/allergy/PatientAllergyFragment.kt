...
import edu.upc.R
import edu.upc.databinding.FragmentPatientAllergyBinding
...
import edu.upc.openmrs.activities.patientdashboard.allergy.PatientAllergyRecyclerViewAdapter.OnLongPressListener
import edu.upc.openmrs.utilities.makeGone
import edu.upc.openmrs.utilities.makeVisible
import edu.upc.openmrs.utilities.observeOnce

@AndroidEntryPoint
class PatientAllergyFragment : edu.upc.openmrs.activities.BaseFragment(), OnLongPressListener, onInputSelected {
    ...
    private fun setupAdapter() {
        with(binding.recyclerViewAllergy) {
            ...
            adapter =
                edu.upc.openmrs.activities.patientdashboard.allergy.PatientAllergyRecyclerViewAdapter(
                    context,
                    emptyList(),
                    this@PatientAllergyFragment
                )
        }
    }
    ...
    private fun showAllergyList(allergies: List<Allergy>) {
        with(binding) {
            ...
            if (allergies.isEmpty()) {
                ...
                (recyclerViewAllergy.adapter as edu.upc.openmrs.activities.patientdashboard.allergy.PatientAllergyRecyclerViewAdapter).updateList(allergies)
    ...
    override fun showDialogueBox(allergy: Allergy) {
        ...
        val dialogList = ArrayList<edu.upc.openmrs.activities.dialog.CustomDialogModel>().apply {
            add(
                edu.upc.openmrs.activities.dialog.CustomDialogModel(
                    getString(R.string.update_allergy_dialog),
                    R.drawable.ic_allergy_edit
                )
            )
            add(
                edu.upc.openmrs.activities.dialog.CustomDialogModel(
                    getString(R.string.delete_allergy_dialog),
