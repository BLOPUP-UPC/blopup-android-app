...
class NewVisitRepository @Inject constructor(
    ...
    suspend fun endVisit(visitId: UUID): Boolean =
        withContext(Dispatchers.IO) {
            ...
            val call: Call<OpenMRSVisit> =
                restApi.endVisitByUUID(visitId.toString(), visitWithEndDate)
    ...
    fun startVisit(patient: Patient): Visit? {
        ...
        if (response.isSuccessful) {
            ...
            val vitalsObservations =
                newVisit?.encounters?.find { it.encounterType?.display == EncounterType.VITALS }?.observations
            val bloodPressure =
                vitalsObservations.let { vitals ->
                    val systolic =
            ...
            val visit =
                newVisit?.startDatetime?.let { DateUtils.parseLocalDateFromOpenmrsDate(it) }
                    ?.let { localDateTime ->
                        Visit(
                            UUID.fromString(newVisit.uuid),
                            UUID.fromString(newVisit.patient.uuid),
                            newVisit.location.display ?: "",
                            localDateTime,
                            bloodPressure,
                            height,
                            weight
